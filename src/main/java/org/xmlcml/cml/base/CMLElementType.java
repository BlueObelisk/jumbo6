...
public class CMLElementType implements CMLConstants {
	private static Logger LOG = Logger.getLogger(CMLElementType.class);
	static {
	...
	private void processContentTypes() {
		...
		if (complexType == null) {
			throw new CMLRuntimeException("No complexType for: "+name);
		}
	...
	public void processAttributes(Map<String, CMLAttribute> attributeGroupMap) {
		...
		if (simpleType != null) {
			...
			if (extensions.size() == 0) {
				throw new CMLRuntimeException("Expected extension children for simpleContent for "+name);
			}
		...
		if (attributes.size() != 0) {
			throw new CMLRuntimeException("Use attributeGroups instead of attributes for "+name);
		}
		List<Node> attributeGroups = CMLUtil.getQueryNodes(
			    contentType, "./"+XSD_ATTRIBUTE_GROUP, XPATH_XSD);
		if (attributeGroups.size() == 0) {
			throw new CMLRuntimeException("NOTE: no attributeGroups for "+name);
		}
	    for (Node node : attributeGroups) {
	    	...
	    	if (attribute == null) {
	    		throw new CMLRuntimeException("cannot find attributeGroup: "+attributeGroupRef);
	    	}
	...
	public void processComplexContent(Map<String, CMLElementType> elementTypeMap) {
		if (simpleType == null) {
			...
			for (Node node : elements) {
				...
				if (ref == null) {
					throw new CMLRuntimeException("No ref attribute for child element in "+name);
				}
				CMLElementType elementType = elementTypeMap.get(ref);
				if (elementType == null) {
					throw new CMLRuntimeException("No element known for "+ref);
				}
	...
	public void processSimpleContent(Map<String, CMLType> typeMap) {
		if (extension != null) {
			...
			if (base == null) {
				throw new CMLRuntimeException("No base given for extension in "+name);
			}
			simpleContentType = typeMap.get(base);
			if (simpleContentType == null) {
				throw new CMLRuntimeException("No type found for: "+base);
			}
		}
	}
