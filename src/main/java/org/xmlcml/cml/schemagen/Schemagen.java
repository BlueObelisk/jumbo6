...
// ==================== command processing ==================	
	...
	protected void parseCommandLineOptions(CommandLine line, Options options) throws Exception {
		if (line.hasOption(HELP_OPT_NAME) ||
    ...
    protected Options getCommandLineOptions() {   	
    	...
    	Option schemaDirOpt = OptionBuilder.withArgName(SCHEMADIR_ARG)
    	.hasArg()
    	.withDescription("directory with schema components")
    	.create(SCHEMADIR_OPT_NAME);
    	options.addOption(schemaDirOpt);
    	
    	Option typeDirOpt = OptionBuilder.withArgName(TYPEDIR_ARG)
    	.hasArg()
    	.withDescription("directory with type components (relative to schema);" +
    			" default 'types' ")
    	.create(TYPEDIR_OPT_NAME);
    	options.addOption(typeDirOpt);
    	
    	Option attributeDirOpt = OptionBuilder.withArgName(ATTRIBUTEDIR_ARG)
    	.hasArg()
    	.withDescription("directory with attribute components (relative to schema);" +
    			" default 'attributes' ")
    	.create(ATTRIBUTEDIR_OPT_NAME);
    	options.addOption(attributeDirOpt);
    	
    	Option elementDirOpt = OptionBuilder.withArgName(ELEMENTDIR_ARG)
    	.hasArg()
    	.withDescription("directory with element components (relative to schema);" +
    			" default 'elements' ")
    	.create(ELEMENTDIR_OPT_NAME);
    	options.addOption(elementDirOpt);
    	
    	Option schemaFileOpt = OptionBuilder.withArgName(SCHEMAFILE_ARG)
    	.hasArg()
    	.withDescription("output schema filename (default schema.xsd)")
    	.create(SCHEMAFILE_OPT_NAME);
    	options.addOption(schemaFileOpt);

    	Option outdirOpt = OptionBuilder.withArgName(OUTDIR_ARG)
    	.hasArg()
    	.withDescription("output directory for schemas")
    	.create(OUTDIR_OPT_NAME);
    	options.addOption(outdirOpt);

    	Option codeOutdirOpt = OptionBuilder.withArgName(CODE_OUTDIR_ARG)
    	.hasArg()
    	.withDescription("output directory for code")
    	.create(CODE_OUTDIR_OPT_NAME);
    	options.addOption(codeOutdirOpt);

    	return options;
    }

// =========== read raw components, assemble and save ===================    
	private void assembleCompleteSchema() throws Exception {
		schema = new Element("xsd:schema", XSD_NS);
		Elements elements = 
			elementGenerator.getSchema().getChildElements();
		for (int i = 0; i < elements.size(); i++) {
			Element copy = (Element) elements.get(i).copy();
			schema.appendChild(copy);
		}
		Elements attributes = 
			attributeGenerator.getSchema().getChildElements();
		for (int i = 0; i < attributes.size(); i++) {
			Element copy = (Element) attributes.get(i).copy();
			schema.appendChild(copy);
		}
		Elements types = 
			typeGenerator.getSchema().getChildElements();
...
// =========================== java code generation ========================	
	...
	private void writeClassHeader(Writer w, String name) throws IOException {
		w.write("" +
		"public abstract class "+CMLUtil.makeAbstractName(name)+" extends CMLElement {\n" +
		"");
	}
...
// ================== attribute accessors ====================	
	...
	public void writeSetFooString(Writer w, String setName) throws IOException {
		...
		"            "+attributeVariable+" = ("+attClassName+") " +
		"attributeFactory.getAttribute(" +
		"\""+attributeName+"\", \""+elementName+"\");\n" +
		"            if ("+attributeVariable+" == null) {\n" +
		"                throw new CMLRuntimeException(\"BUG: cannot process attributeGroupName : "+attributeName+
		" probably incompatible attributeGroupName and attributeName\");\n"+
		"            }\n" +
		"        }\n" +
...
//	 ======================== text content ====================	
	...
	public void writeXMLContentDeclaration(Writer w, String setName) throws IOException {
		w.write("" +
		"    "+attClassName+" "+attributeVariable+";\n" +
		"");
	}
