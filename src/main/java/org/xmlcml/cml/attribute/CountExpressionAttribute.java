...
public class CountExpressionAttribute extends StringSTAttribute {
     ...
     */
    public CountExpressionAttribute(Attribute att) throws CMLRuntimeException {
        super(att);
    }
    ...
    public void setCMLValue(String value) throws CMLRuntimeException {
        if (value == null) {
            throw new CMLRuntimeException("null CountExpressionAttribute value");
     ...
     */
    public static void generateAndInsertClones(CMLElement element) throws CMLRuntimeException {
        if (element == null) {
...
//            }
            if (countEx.startsWith("gaussian")) {
                ...
                if (counts.length != 2 || counts[0] <= 3 || counts[1] < 1 || 
                        counts[0] / counts[1] < 3) {
                    throw new CMLRuntimeException("Bad gaussian: "+countEx);
                }
                throw new CMLRuntimeException("gaussian NYI");
            } else if (countEx.startsWith("range")) {
                countEx = countEx.substring("range".length());
                int[] counts = parseCount(countEx);
                if (counts.length != 2 || counts[0] <= 0 || counts[0] >= counts[1]) {
                    throw new CMLRuntimeException("Bad range: "+countEx);
                }
                ...
                throw new CMLRuntimeException("Bad count expression: "+countEx);
            }
        }
        return count;
    }
    ...
    private int[] parseCount(String countEx) {
        if (!countEx.startsWith(S_LBRAK) ||
                ...
                throw new CMLRuntimeException("Bad range: requires (...): "+countEx);
        }
        ...
        for (int i = 0; i < countStrings.length; i++) {
            try {
                ...
                throw new CMLRuntimeException("Bad integer in countExpression :"+countStrings[i]);
            }
        }
        return counts;
    }
