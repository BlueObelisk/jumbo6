...
public class DelimiterAttribute extends StringSTAttribute {
     ...
     */
    public DelimiterAttribute(Attribute att) throws CMLRuntimeException {
        super(att);
    ...
    public void setCMLValue(String value) throws CMLRuntimeException {
        ...
        if (value.equals(S_EMPTY)) {
        	...
        	throw new CMLRuntimeException("Non-whitespace delimiter must only be single character");
        }
     ...
     * @throws CMLRuntimeException if d is part of s
     */
    public void checkDelimiter(String s) throws CMLRuntimeException {
        if (s.split(splitter).length > 1) {
            throw new CMLRuntimeException("cannot delimit {" + s + "} with {" + concat + S_RCURLY);
        }
    }
