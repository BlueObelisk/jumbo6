...
public class CMLArray extends AbstractArray implements HasUnits, HasArraySize {
	...
	public void finishMakingElement(Element parent) {
		...
		if (dataType.equals(XSD_STRING)) {
			...
			try {
				...
				throw new CMLRuntimeException("Fault in XML: "
		...
		if (this.getDataType().equals(XSD_INTEGER)) {
			...
			for (int i = 0; i < ii.length; i++) {
				try {
					...
					throw new CMLRuntimeException("Bad int (" + ss[i]
							+ ") at position: " + i);
				}
			}
		}
	...
	public void append(String s) throws CMLRuntimeException {
		...
		if (!XSD_STRING.equals(dataType)) {
			throw new CMLRuntimeException("Cannot add string (" + s
	...
	public void append(double d) throws CMLRuntimeException {
		...
		if (!XSD_DOUBLE.equals(dataType)) {
			throw new CMLRuntimeException("Cannot add double to array of: "
					+ dataType);
		}
		appendXML(S_EMPTY + d);
	}
	...
	public void append(int i) throws CMLRuntimeException {
		...
		if (!XSD_INTEGER.equals(dataType)) {
			throw new CMLRuntimeException("Cannot add int to array of: "
					+ dataType);
		}
		appendXML(S_EMPTY + i);
	}
