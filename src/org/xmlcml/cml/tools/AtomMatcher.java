...
public class AtomMatcher extends AbstractTool {
    ...
    public CMLMap mapAtomSets(CMLAtomSet atomSet0, CMLAtomSet atomSet1)
        ...
        if (this.getAtomMatchStrategy().equals(
            ...
            if (morganList0.size() != morganList1.size()) {
                ...
                for (int i = 0; i < morganList0.size(); i++) {
                    ...
                    if (atomSetx0.size() == atomSetx1.size()) {
                        cmlLink = new CMLLink();
                        cmlLink.setTitle(title + " " + atomSetx0.size());
            ...
            Map uniqueAtomMap0 = AtomTree.getAtomTreeLabelling(atomSet0, this);
            Map uniqueAtomMap1 = AtomTree.getAtomTreeLabelling(atomSet1, this);
            if (uniqueAtomMap0.size() != 0 || uniqueAtomMap0.size() != 0) {
                ...
                for (Iterator it = keySet0.iterator(); it.hasNext();) {
                    ...
                    if (object1 == null) {
                        ...
                        cmlLink = new CMLLink();
                        cmlLink.setTitle(title + " " + atomSet0.size());
     ...
     * @throws CMLException
     */
     public CMLMap getUniqueMatchedAtoms(CMLAtomSet atomSet, CMLAtomSet targetAtomSet,
            AtomMatcher atomMatcher) throws CMLException {
        // note unique atoms
        Map uniqueAtomMap0 = AtomTree.getAtomTreeLabelling(atomSet, atomMatcher);
        Map uniqueAtomMap1 = AtomTree.getAtomTreeLabelling(targetAtomSet,
                atomMatcher);
        CMLMap cmlMap = new CMLMap();
        cmlMap.setToType(CMLAtom.TAG);
        cmlMap.setFromType(CMLAtom.TAG);
    ...
    public CMLMap mapGeometricalNeighbours(CMLAtomSet atomSet,
        ...
        if (atomSet.size() != atomSet2.size()) {
            ...
            for (int j = 0; j < nAtoms2; j++) {
                ...
                // System.out.print("
                // "+atom2Id[j]+"("+atom2.getElementType()+")");
            }
            logger.info("");
            for (int i = 0; i < nAtoms1; i++) {
                CMLAtom atom1 = (CMLAtom) atomSet.getAtoms().get(i);
                atom1Id[i] = atom1.getId();
                logger.info(" " + atom1Id[i] + "(" + atom1.getElementType()
                        + ")");
    ...
    public void splitAndProcessAtomSets(CMLMap map, CMLAtomSet fromAtomSet,
        ...
        for (CMLLink atomSetLink : atomSetLinks) {
            ...
            String fromSet = Util.concatenate(atomSetLink.getFromSet(), " ");
            String toSet = Util.concatenate(atomSetLink.getToSet(), " ");
    ...
    public CMLMap matchProductsToNextReactants(CMLReaction reaction0,
        ...
        for (int i = 0; i < molecule0.size(); i++) {
            CMLMolecule mol0 = molecule0.get(i);
            if (mol0 == null) {
                logger.severe("Null molecule: molecule0[" + i + "]");
                continue;
            }
            for (int j = 0; j < molecule1.size(); j++) {
                CMLMolecule mol1 = molecule1.get(j);
                if (mol1 == null) {
                    logger.severe("Null molecule: molecule1[" + j + "]");
                    continue;
                }
    ...
    public List<AtomPair> translateProductsToReactants(CMLReaction reaction)
        ...
        // logger.info("R/P" + reactantCount+"/"+productCount);
        // start easy...
        if (reactantCount == 1 && productCount == 1) {
            ...
            // + ") to product (" + reactant.getAtomCount() + ")");
        } else {
            ...
            // logger.info("XXR/P" +
            // reactantAtoms.length+"/"+productAtoms.length);
    ...
    public void moveReactantProductToSpectator(CMLReaction reaction)
        ...
        for (int i = 0; i < spectators.size(); i++) {
            ...
            if (ref != null) {
                ...
                // ("+reactantMolecule.getId()+"/"+reactantTool.getAtomCount()+")
                // and product
                // ("+productMolecule.getId()+"/"+productTool.getAtomCount()+")
                ...
                try {
                    ...
                    logger.severe("Reactant (" + reactantMolecule.getId() + "/"
