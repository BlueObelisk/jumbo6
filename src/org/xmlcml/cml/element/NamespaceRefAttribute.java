...
public abstract class NamespaceRefAttribute extends StringAttribute {
     ...
     * @param name
     */
    protected NamespaceRefAttribute(String name) {
        super(name);
        init(name);
    }

    protected NamespaceRefAttribute(String name, String value) {
        this(name);
        this.setCMLValue(value);
    }

    protected NamespaceRefAttribute(Attribute att) {
    ...
    public void setCMLValue(String value) {
        ...
        if (this.getValue().equals(S_EMPTY)) {
            ...
            throw new CMLRuntimeException("attribute value [" + this.getValue()
     ...
     *            to search under
     * @param name
     *            of descendant elements (null == wildcard)
     * @param attributeName
     *            in descendant elements (null == wildcard)
     * @param dictionaryMap
...
//            rootElement = new Builder().build(file).getRootElement();
    ...
    public GenericEntry getEntry(GenericDictionaryMap dictionaryMap) {
        if (dictionaryMap == null) {
            ...
            throw new CMLRuntimeException("Null dictionaryMap");
        }
        GenericDictionary dictionary = null;
        String namespace = this.getNamespaceURIString();
        if (namespace != null) {
            ...
            throw new CMLRuntimeException("Unknown namespace for " + this.getValue());
        }
    ...
    public static String createValue(String prefix, String value) {
        if (prefix == null) {
            throw new CMLRuntimeException("null prefix");
        }
        if (value == null) {
            throw new CMLRuntimeException("null value");
        }
        if (prefix.trim().equals("")) {
            throw new CMLRuntimeException("cannot have empty prefix");
        }
        if (value.trim().equals("")) {
            throw new CMLRuntimeException("cannot have empty value");
        }
        if (!prefix.matches(PREFIX_REGEX)) {
            throw new CMLRuntimeException("Prefix [" + prefix + "] incompatible with "
                    + PREFIX_REGEX);
        }
