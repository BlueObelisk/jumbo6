...
public class CMLArg extends AbstractArg {
    ...
    public static void addArg(CMLElement parent, CMLArg arg, int position) 
        ...
        if (position > args.size()) {
            throw new CMLRuntimeException("position > argCount");
        }
        String argName = arg.getName();
        for (int i = 0; i < args.size(); i++) {
            String name = ((CMLArg)args.get(i)).getName();
            if (name == null || name.trim().equals(S_EMPTY)) {
                throw new CMLRuntimeException("missing name in arg: ");
            }
            if (name.equals(argName)) {
                throw new CMLRuntimeException("duplicate arg: "+argName);
            }
        }
        // OK, insert arg
...
//     */
        ...
        for (CMLArg fromArg : fromList) {
            ...
            for (CMLArg toArg : toList) {
                ...
                if (fromName.equals(toName)) {
                    ...
                    if (idx < 0) {
                        throw new CMLRuntimeException("BUG idx < 0");
                    }
            ...
            if (this.getDataType().equals(XSD_STRING)) {
                ...
                if (this.getDataType().equals(XSD_DOUBLE)) {
                    try {
                        ...
                        for (int i = 1; i < tokens.length; i+=2) {
                            ...
                            if (op.equals(S_PLUS)) {
                                ...
                                throw new CMLRuntimeException("Bad operator: ("+op+") in eval: "+value);
                            }
                        }
                        this.setDouble(dd);
                    } catch (NumberFormatException e) {
                        throw new CMLRuntimeException("Bad numeric double eval: "+value);
                    }
                    ...
                    try {
                        ...
                        throw new CMLRuntimeException("Bad numeric integer eval: "+value);
                    }
                }
            }
        }
     ...
     * @throws CMLRuntimeException if no integer scalar
     */
    public int getInteger() throws CMLRuntimeException {
        CMLScalar scalar = getScalar();
        if (scalar == null) {
            throw new CMLRuntimeException("no child scalar");
        }
     ...
     * @throws CMLRuntimeException
     */
    public void setInteger(int i) throws CMLRuntimeException {
     ...
     * @return string value
     * @throws CMLRuntimeException if no string scalar
     */
    public String getString() throws CMLRuntimeException {
        String string = null;
        CMLScalar scalar = getScalar();
        if (scalar != null) {
     ...
     * @throws CMLRuntimeException
     */
    public void setString(String s) throws CMLRuntimeException {
        CMLScalar scalar = (CMLScalar) this.getChildCMLElement(CMLScalar.TAG, 0);
        if (scalar == null) {
     ...
     * @return double value
     * @throws CMLRuntimeException if no double scalar
     */
    public double getDouble() throws CMLRuntimeException {
        CMLScalar scalar = getScalar();
        if (scalar == null) {
            throw new CMLRuntimeException("no child scalar");
        }
     ...
     * @throws CMLRuntimeException
     */
    public void setDouble(double d) throws CMLRuntimeException {
        CMLScalar scalar = (CMLScalar) this.getChildCMLElement(CMLScalar.TAG, 0);
        if (scalar == null) {
        ...
        for (int i = 0; i < nodes.size(); i++) {
            ...
            if (id == null) {
                ...
                if (idSet.contains(id)) {
                    throw new CMLRuntimeException("Non-unique element id: "+id);
                }
            }
