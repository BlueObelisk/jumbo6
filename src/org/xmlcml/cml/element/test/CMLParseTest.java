...
public class CMLParseTest extends AbstractTest {
    ...
    public void testBasicParse() {
        ...
        // as well
        /*-- deliberate error, but omit to avoid confusion
         String xml2S = "<foo1>";
         try {
         builder.build(new StringReader(xml2S));
         Assert.fail("should throw parse exception ");
         } catch (ValidityException e) {
         Assert.fail("should not throw validity exception "+e.getMessage());
         } catch (ParsingException e) {
         Assert.assertEquals("parse exception ", "XML document structures must start and end within the same entity.", e.getMessage());
         } catch (IOException e) {
         Assert.fail("should not throw IO exception "+e.getMessage());
         } catch (CMLRuntime e) {
        ...
        /*-- deliberate error, but omit to avoid confusion
         ...
         Assert.assertEquals("parse exception ", "The markup in the document preceding the root element must be well-formed.", e.getMessage());
         } catch (IOException e) {
         Assert.fail("should not throw IO exception "+e.getMessage());
         } catch (CMLRuntime e) {
         Assert.fail("should not throw CMLRuntime exception "+e.getMessage());
