...
public class CMLUnitList extends AbstractUnitList implements GenericDictionary {
	 ...
	 * @return CMLUnitList
	 */
	public static CMLUnitList makeElementInContext(Element parent) {
	 ...
	 *             if file is not a well-formed unitList
	 */
	public static CMLUnitList createUnitList(URL url) throws IOException,
			CMLException {
		Document dictDoc = null;
		InputStream in = null;
		// this will fail if unitList is badly formed
		try {
			in = url.openStream();
	...
	public GenericEntry getGenericEntry(String id) {
		return (getUnit(id));
		// throw new CMLRuntime("Use getUnit() instead");
	}
	...
	/*--
	 private void indexDomEntries() {
	 if (unitMap == null) {
	 unitMap = new HashMap<String, CMLUnit>();
	 CMLElements<CMLUnit> unitNodes = this.getUnitElements();
	 for (CMLUnit unit : unitNodes) {
	 String id = unit.getId();
	 if (id != null) {
	 unitMap.put(id, unit);
	 }
	...
	public GenericDictionary createDictionary(URL url) throws IOException {
		...
		if (dictDoc != null) {
			...
			if (root instanceof CMLUnitList) {
				...
				throw new CMLRuntimeException("Expected CMLUnitList root element in ("
						+ url + "), found: " + root.getClass().getName() + "/"
						+ root.getLocalName());
			}
		}
		if (dt != null) {
	...
	public CMLUnitList getSIUnitList() {
		if (siUnitList == null) {
			...
			if (siNamespace != null) {
				...
				siUnitList = (CMLUnitList) unitListMap
						.getDictionary(siNamespace);
				if (siUnitList != null) {
