...
public class ElementTest extends BaseTest {
	...
	private void xomNoValidate(String file) throws IOException, ValidityException, ParsingException {
		Document doc = null;
	...
	private void xomXercesValidate(String file) throws SAXException, IOException, ValidityException, ParsingException {
...
/*	@Test
	public void testXercesValidateXML() {
		xercesValidate(cml0);
		// this throws an error as it has no schema
		// xercesValidate(noSchema);
	}
*/
	/**
	...
	public void testParseXomNoSchema1() {
        Util.output("  === xom Parse, no schema, no validation: "
				+ noSchema + " ====");
		InputStream in = null;
		Document doc = null;
		try {
			in = Util.getInputStreamFromResource(BASE_RESOURCE + U_S
					+ noSchema);
			doc = new Builder().build(in);
			Assert.assertNotNull("document", doc);
