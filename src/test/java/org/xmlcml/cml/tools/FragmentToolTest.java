...
public class FragmentToolTest extends AbstractTest {
	...
	private Catalog moleculeCatalog = null;
 	...
 	/**
 	 * Test method for {@link org.xmlcml.cml.tools.FragmentTool#processBasic(org.xmlcml.cml.tools.Catalog)}.
...
//   		  </propertyList>
 	...
 	private Document readDocument(String fileroot) throws FileNotFoundException, CMLRuntimeException {
 		Document doc = null;
 		try {
 			InputStream is = Util.getInputStreamFromResource("org/xmlcml/cml/tools/examples/molecules/fragments/"+fileroot+".xml");
...
//	 			e.printStackTrace();
 	...
 	private void test(boolean debug, boolean check, int serial, boolean checkIE) {
 		...
 		generateElement(basicRoot, debug, serial,
 		 		intermediateRoot, explicitRoot, completeRoot, check);
 	}

 	private void generateElement(String basicRootName, boolean debug, int serial,
 			String intermediateRootName, 
 			String explicitRootName, 
 			String completeRootName, boolean check) {
 		...
 		generateElement(fragment, debug, serial,
 			intermediate, explicit, complete, check);
 	}
 	
 	private void generateElement(CMLElement fragment, boolean debug, int serial,
 			CMLElement intermediate, CMLElement explicit, CMLElement complete,
 			boolean check) {
 		...
 		if (generatedElement == null) {
 			...
 			runMarkush((CMLFragmentList) generatedElement, debug, serial, 
				intermediate, explicit, complete, check);
 		}
 	}
 	...
 	private void dump(Element element, String filename) {
 		...
 		"java"+File.separator+
 		"org"+File.separator+
 		"xmlcml"+File.separator+
 		"cml"+File.separator+
 		"tools"+File.separator+
 		"examples"+File.separator+
 		"molecules"+File.separator+
 		"fragments";
 		try {
 	...
 	private void runMarkush(CMLFragmentList fragmentList, boolean debug, int serial,
...
// 	@Ignore
 	public void test50_57() {
 		boolean check = false;
 		boolean debug = true;
 		test(check, debug, 50, true);
 		test(check, debug, 51, true);
 		test(check, debug, 52, true);
 		test(check, debug, 53, true);
 		test(check, debug, 54, true);
 		test(check, debug, 56, true);
 		test(check, debug, 57, true);
