...
public class CMLParameterListTest extends AbstractTest {
    ...
    public void testCopy() {
        CMLParameterList parameterListX = (CMLParameterList) parameterList.copy();
    ...
    public void testGetParameterDescendantsCMLElement() {
        List<CMLParameter> paramList = 
            CMLParameterList.getParameterDescendants(parameterList);
        Assert.assertEquals("param list", 2, paramList.size());
        // note order is variable
        Assert.assertEquals("param 0", "foo:mpt", paramList.get(0).getName());
        Assert.assertEquals("param 1", "foo:angle", paramList.get(1).getName());
    ...
    /**
     * Test method for
     * 'org.xmlcml.cml.element.CMLParameterList.getParameterDescendantsByDictRef(List<CMLParameter>,
     * String)'
     */
    @Test
    public void testGetParameterDescendantsByDictRef() {
        List<CMLParameter> paramList = 
            CMLParameterList.getParameterDescendants(parameterList);
        Assert.assertEquals("param list", 2, paramList.size());
        List<CMLParameter> paramListX = 
            CMLParameterList.getParameterDescendantsByDictRef(paramList, "bar:xxx");
        Assert.assertEquals("param list", 1, paramListX.size());
        Assert.assertEquals("param 0", "foo:angle", paramListX.get(0).getName());
    }
    ...
    public void testGetParameterDescendantsByName() {
        List<CMLParameter> paramListX = 
            parameterList.getParameterDescendantsByName("foo:angle");
        Assert.assertEquals("param list", 1, paramListX.size());
        Assert.assertEquals("param 0", "foo:angle", paramListX.get(0).getName());
    }
