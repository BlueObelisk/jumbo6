...
public class CMLElementTest extends AbstractTest {
    ...
    public void testElement() {
        ...
        /*-- omit deliberate error
         System.err.println("deliberate error:");
         String test2S = "<cml>";
         Document doc2 = null;
         try {
         doc2 = new Builder().build(new StringReader(test2S));
         Assert.fail("should throw parsing exception");
         } catch (ValidityException e) {
         Assert.fail("should not throw "+e);
         } catch (ParsingException e) {
         Assert.assertEquals("should throw ", "XML document structures must start and end within the same entity.", e.getMessage());
         } catch (IOException e) {
         Assert.fail("should not throw "+e);
         }
         Assert.assertNull("test2 ", doc2);
         System.err.println("end deliberate error:");
         --*/
...
//        Element cml = (Element) new Builder().build(new StringReader(s)).getRootElement();
